...
%% enumerated types
-type 'Genre'() :: 'ALL' | 'SYMPHONIC_METAL' | 'MELODIC_DEATH_METAL' | 'POWER_METAL' | 'THRASH_METAL' | 'ACAPECCLA_METAL' | 'PROG_METAL'.
-export_type(['Genre'/0]).
...
-type 'SubscriptionRequest'() ::
      #{'City'                  => [iodata()],      % = 1
        'Band'                  => [iodata()],      % = 2
        'Genre'                 => ['ALL' | 'SYMPHONIC_METAL' | 'MELODIC_DEATH_METAL' | 'POWER_METAL' | 'THRASH_METAL' | 'ACAPECCLA_METAL' | 'PROG_METAL' | integer()] % = 3, enum Genre
       }.
...
e_enum_Genre('THRASH_METAL', Bin, _TrUserData) ->
    <<Bin/binary, 4>>;
e_enum_Genre('ACAPECCLA_METAL', Bin, _TrUserData) ->
    <<Bin/binary, 5>>;
...
d_enum_Genre(5) -> 'ACAPECCLA_METAL';
...
v_enum_Genre('ACAPECCLA_METAL', _Path, _TrUserData) ->
    ok;
...
enum_symbol_by_value_Genre(5) -> 'ACAPECCLA_METAL';
...
enum_value_by_symbol_Genre('ACAPECCLA_METAL') -> 5;
...
source_basename() -> "concert_notifier.proto".
...
get_all_source_basenames() -> ["concert_notifier.proto"].
...
%% functions.
get_all_proto_names() -> ["concert_notifier"].


get_msg_containment("concert_notifier") ->
    ['ConcertEvent', 'SubscriptionRequest'];
get_msg_containment(P) ->
    error({gpb_error, {badproto, P}}).


get_pkg_containment("concert_notifier") -> undefined;
get_pkg_containment(P) ->
    error({gpb_error, {badproto, P}}).


get_service_containment("concert_notifier") ->
    ['ConcertNotifier'];
get_service_containment(P) ->
    error({gpb_error, {badproto, P}}).


get_rpc_containment("concert_notifier") ->
    [{'ConcertNotifier', 'TrackConcerts'}];
get_rpc_containment(P) ->
    error({gpb_error, {badproto, P}}).


get_enum_containment("concert_notifier") -> ['Genre'];
